...
function _UploadFile(gameObjectNamePtr, callbackMethodNamePtr, filterPtr, multiSelect) {
 ...
 fileInput.onchange = function(event) {
  ...
  reader.onload = function(event) {
   window.unityInstance.SendMessage(gameObjectName, callbackMethodName, event.target.result);
  };
  for (var i = 0; i < event.target.files.length; i++) {
   reader.readAsText(event.target.files[i]);
  }
  if (fileInput.parentElement != null) {
